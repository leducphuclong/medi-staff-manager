...
//            "SELECT nv.*, cv.TenChucVu, pb.TenPhongBan " +
//            "FROM nhan_vien nv " +
//            "LEFT JOIN chuc_vu cv ON nv.IDChucVu = cv.IDChucVu " +
...
//        );
//
//        // Khớp criteria với giá trị từ HTML (thường là tên thuộc tính)
...
//                break;
//            // Bạn có thể muốn tìm theo tenPhongBan hoặc tenChucVu nếu cần
//            // case "tenphongban":
//            //     queryBuilder.append("LOWER(pb.TenPhongBan) LIKE LOWER(?)");
//            //     break;
//            default:
//                 System.err.println("Search criteria not recognized: " + criteria + ". Defaulting to HoTen search.");
//                 queryBuilder.append("LOWER(nv.HoTen) LIKE LOWER(?)"); // Hoặc ném lỗi
//                 // Hoặc nếu muốn tìm trên nhiều trường:
//                 // queryBuilder.append("(LOWER(nv.HoTen) LIKE LOWER(?) OR nv.CCCD LIKE ? OR nv.SDT LIKE ? OR LOWER(nv.Email) LIKE LOWER(?))");
//                 break;
//        }
...
//            // Nếu default tìm trên nhiều trường, bạn cần set nhiều tham số hơn
//            // if (criteria.equalsIgnoreCase("default_multi_field_search")) {
//            //    stmt.setString(1, searchPattern);
...
//            }
//            // Sử dụng getOrCreate để đảm bảo ID hợp lệ hoặc tạo mới nếu cần
//            stmt.setInt(7, getOrCreateChucVu(employee.getTenChucVu()));
...
//    }
//
//    // Giả sử IDNhanVien là auto-increment trong CSDL
...
//        // Nếu currentEmployeeId là null, nghĩa là đang thêm mới, không cần loại trừ.
//        // Nếu currentEmployeeId khác null, nghĩa là đang cập nhật, cần loại trừ nhân viên hiện tại.
...
//        if (tenChucVu == null || tenChucVu.trim().isEmpty()) return -1; // Hoặc ném lỗi nếu tên không được null
...
//    }
//
//    public int getOrCreateChucVu(String tenChucVu) throws SQLException {
...
//            return idChucVu;
//        }
//        // Thêm mới nếu không tồn tại (giả sử HeSoLuong mặc định là 1.0)
...
//    }
//
//    // Các phương thức để lấy danh sách tên Chức Vụ/Phòng Ban cho dropdown
