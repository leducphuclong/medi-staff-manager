...
public class NhanVienDAO2 {
	...
	public List<NhanVienBEAN> layToanBoNhanVien() {
		List<NhanVienBEAN> employees = new ArrayList<>();
		...
		try (PreparedStatement stmt = connection.prepareStatement(query); ResultSet rs = stmt.executeQuery()) {
			...
			while (rs.next()) {
				...
				NhanVienBEAN nhanVien = new NhanVienBEAN(idNhanVien, cccd, hoTen, sdt, email, gioiTinh, ngaySinh, idChucVu,
	...
	public NhanVienBEAN getNhanVienById(int idNhanVien) {
		String query = "SELECT nv.IDNhanVien, nv.CCCD, nv.HoTen, nv.Sdt, nv.Email, nv.GioiTinh, nv.NgaySinh, "
				+ "nv.IDChucVu, nv.IDPhongBan, cv.TenChucVu, pb.TenPhongBan " + "FROM nhan_vien nv "
				+ "JOIN chuc_vu cv ON nv.IDChucVu = cv.IDChucVu "
		...
		try (PreparedStatement stmt = connection.prepareStatement(query)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					...
					return new NhanVienBEAN(idNhanVien, cccd, hoTen, sdt, email, gioiTinh, ngaySinh, idChucVu, idPhongBan,
	...
	public List<NhanVienBEAN> layNhanVienTheoIdPhongBan(int idPhongBan) {
		List<NhanVienBEAN> danhSach = new ArrayList<>();
		String query = "SELECT nv.IDNhanVien, nv.CCCD, nv.HoTen, nv.Sdt, nv.Email, nv.GioiTinh, nv.NgaySinh, "
				+ "nv.IDChucVu, nv.IDPhongBan, cv.TenChucVu, pb.TenPhongBan " + "FROM nhan_vien nv "
				+ "JOIN chuc_vu cv ON nv.IDChucVu = cv.IDChucVu "
		...
		try (PreparedStatement stmt = connection.prepareStatement(query)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				while (rs.next()) {
					...
					NhanVienBEAN nv = new NhanVienBEAN(idNhanVien, cccd, hoTen, sdt, email, gioiTinh, ngaySinh, idChucVu,
							idPhongBan, tenChucVu, tenPhongBan);
					danhSach.add(nv);
				}
			}
