...
function initializeDepartmentPage() {
	bridge.log("Chuẩn bị khởi tạo trang Phòng ban")
...
function handleSearch() {
    ...
    bridge.getQuanLyPhongBanBridge().log(`Tìm kiếm phòng ban: loại=${searchType}, giá trị=${searchValue}`);
    
    try {
        let resultJson = "";
        if (searchType === "ten") {
            resultJson = bridge.getQuanLyPhongBanBridge().timKiemPhongBanTheoTen(searchValue);
        } else if (searchType === "id") {
            if (isNaN(parseInt(searchValue))) {
                showNotification("ID phải là số nguyên", "error");
                return;
            }
            resultJson = bridge.getQuanLyPhongBanBridge().timKiemPhongBanTheoId(searchValue);
        }
        ...
        bridge.getQuanLyPhongBanBridge().log("Lỗi khi tìm kiếm: " + e);
...
function displaySearchResults(results) {
    ...
    results.forEach(dept => {
        if (typeof dept.idPhongBan === 'undefined' || typeof dept.tenPhongBan === 'undefined') {
            bridge.getQuanLyPhongBanBridge().log("Dữ liệu phòng ban không hợp lệ từ kết quả tìm kiếm:");
            return;
        }
        ...
        row.addEventListener('click', (event) => {
            ...
            if (typeof window.bridge.getQuanLyPhongBanBridge() !== 'undefined' && typeof window.bridge.getQuanLyPhongBanBridge().navigateToDepartmentDetail === 'function') {
                window.bridge.getQuanLyPhongBanBridge().navigateToDepartmentDetail(dept.idPhongBan, dept.tenPhongBan, bridge.getWebView());
...
function loadDepartmentTableData() {
    bridge.log("Đang lấy dữ liệu các phòng ban vào bảng");
  
    try {
        const departmentsJson = window.bridge.getQuanLyPhongBanBridge().layDanhSachPhongBan();
		bridge.log("đã lấy danh sách phòng ban");
        const departments = JSON.parse(departmentsJson); 
		
        const tbody = document.querySelector("#departmentTable tbody");

        if (!tbody) {
            bridge.getQuanLyPhongBanBridge().log("JS: Không tìm thấy tbody cho #departmentTable.");
            return;
        }
        tbody.innerHTML = '';
        ...
        departments.forEach(dept => {
            if (typeof dept.idPhongBan === 'undefined' || typeof dept.tenPhongBan === 'undefined') {
                bridge.getQuanLyPhongBanBridge().log("Dữ liệu phòng ban không hợp lệ từ Java:");
                return;
            }
            ...
            row.addEventListener('click', (event) => {
                if (event.target.tagName === 'BUTTON' || event.target.closest('button')) return;
				bridge.log("Đang vào trang phòng ban chi tiết")
                bridge.getQuanLyPhongBanBridge().navigateToDepartmentDetail(dept.idPhongBan, dept.tenPhongBan, bridge.getWebView());
            });
        });
    } catch (e) {
        bridge.getQuanLyPhongBanBridge().log("JS: Lỗi trong loadDepartmentTableData:", e);
        const tbody = document.querySelector("#departmentTable tbody");
...
function handleEditDepartment(currentId, currentName) {
	...
	bridge.getQuanLyPhongBanBridge().log("JS: handleAddDepartment ĐƯỢC GỌI");
    ...
    function onSave() {
        ...
        if (typeof window.bridge.getQuanLyPhongBanBridge() !== "undefined" && typeof window.bridge.getQuanLyPhongBanBridge().suaPhongBan === "function") {
            if (window.bridge.getQuanLyPhongBanBridge().suaPhongBan(currentId, newId, newName)) {
...
function handleAddDepartment() {
	bridge.getQuanLyPhongBanBridge().log("Da vao handle add");
    ...
    function onSave() {
        ...
        if (typeof window.bridge.getQuanLyPhongBanBridge() !== "undefined" && typeof window.bridge.getQuanLyPhongBanBridge().themPhongBan === "function") {
            if (window.bridge.getQuanLyPhongBanBridge().themPhongBan(newId, newName)) {
...
function handleDeleteDepartment(id, name) {
    bridge.getQuanLyPhongBanBridge().log(`JS: handleDeleteDepartment CALLED - ID: ${id}, Name: "${escapeJsString(name)}"`);
    ...
    showCustomConfirm(confirmMessage, confirmTitle,
        function() { // This is the onOk callback
            ...
            if (typeof window.bridge.getQuanLyPhongBanBridge() !== 'undefined' && typeof window.bridge.getQuanLyPhongBanBridge().xoaPhongBan === 'function') {
                try {
                    const success = window.bridge.getQuanLyPhongBanBridge().xoaPhongBan(id);
                    ...
                    console.error("JS: Error calling bridge.getQuanLyPhongBanBridge().xoaPhongBan:", e);
                    showNotification("Lỗi nghiêm trọng khi gọi hàm xóa phòng ban.", "error");
                }
            } else {
                showNotification("Lỗi kết nối (xoaPhongBan).", "error");
                console.error("JS: bridge.getQuanLyPhongBanBridge().xoaPhongBan is not available.");
            }
        },
