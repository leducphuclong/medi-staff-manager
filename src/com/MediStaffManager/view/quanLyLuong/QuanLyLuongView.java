...
public class QuanLyLuongView extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        QuanLyLuongBridge bridge = new QuanLyLuongBridge(ctrl, this);

        WebView view = new WebView();
        engine = view.getEngine();
        engine.setJavaScriptEnabled(true);

        // Inject alert, prompt handlers tương tự...
        engine.setOnAlert((WebEvent<String> ev) ->
            Platform.runLater(() -> { new Alert(Alert.AlertType.INFORMATION, ev.getData()).showAndWait(); })
        );
        // Khi load xong, inject bridge
        engine.getLoadWorker().stateProperty().addListener((obs, o, n) -> {
            if (n == Worker.State.SUCCEEDED) {
                JSObject win = (JSObject) engine.executeScript("window");
                win.setMember("luongNhanVienBridge", bridge);
                engine.executeScript("if(typeof initializePage==='function') initializePage();");
            }
        });

        // Load HTML quản lý lương
        bridge.loadPage("QuanLyLuong.html");

        primaryStage.setScene(new Scene(new StackPane(view), 1200, 750));
        primaryStage.setTitle("Quản Lý Lương Nhân Viên");
        primaryStage.show();
    }

    public WebEngine getWebEngine() { return engine; }
